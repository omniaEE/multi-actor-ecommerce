  ...
  cartItems.forEach((item) => {
    ...
    li.innerHTML = `
            <div>
              ...
              <br />
              <small>Qty: ${item.amount} <br/> Size : ${
      item.size
    }  <br/ > Color : ${item.color}</small> 
            </div>
...
document.querySelector("form").addEventListener("submit", function (e) {
  ...
  let newOrders = [
    {
      ...
      total: (subtotal - subtotal * (promoapplied ? 0.25 : 0.2) + 15).toFixed(
        2
      ),
      status: "processing",
      orderDate: orderDate,
  ...
  if (insufficientStockItems.length > 0) {
    let productNames = insufficientStockItems
  ...
  if (userIndex !== -1) {
    ...
    allData.users[userIndex].orders.push(
      ...newOrders.map((_, i) => allData.orders.length - newOrders.length + i)
    );
  }

  cartItems.forEach((item) => {
    const productIndex = allData.products.findIndex(
