...
let productsToShow = data.products
  ...
  .slice(0, 4); // limit to 4 others

productsToShow.forEach((product) => {
  //rating
  const avgRating =
    product.ratings.reduce((acc, val) => acc + val, 0) / product.ratings.length;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (product) {
    ...
    document.getElementById("product-price").innerHTML = `$${
      product.price
    } <s>${product.old_price ? "$" + product.old_price : ""}</s> ${
      product.old_price
        ? "<span>-" +
          Math.floor(
            ((product.old_price - product.price) / product.old_price) * 100
    ...
    document.getElementById("product-rate").textContent = (
      product.ratings.reduce((acc, val) => acc + val, 0) /
      product.ratings.length
    ).toPrecision(2);
    document.getElementById("product-stock").innerHTML = `${
      product.stock > 0 ? "Stock : Available" : "<span>Out of Stock</span>"
    }`;
    ...
    for (let i = 0; i < product.colors.length; i++) {
      ...
      if (i == 0) {
        if (
          proColor.style.backgroundColor == "white" ||
          proColor.style.backgroundColor == "beige"
        ) {
      ...
      colors[i].addEventListener("click", () => {
        colors.forEach((color) => {
          if (
            ...
            colors[i].style.backgroundColor == "beige"
          ) {
            color.innerHTML = "";
            colors[
              i
            ].innerHTML = `<i class="fa-solid fa-check" style="color:black;"></i>`;
    ...
    function createReviewCard(review) {
      ...
      reviewElement.innerHTML = `
        ...
        review.rate < 3 ? `style="color:#d90b0b;"` : ""
      } ></i></h3>
                        <p>${review.review}</p>
                        <br />
      ...
      if (loggedUser) {
        ...
        if (existingCartItem) {
          existingCartItem.amount =
            Number(existingCartItem.amount) + Number(counter.innerText);
        } else {
          loggedUser.cart.push({
            product: product,
