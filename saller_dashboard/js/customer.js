...
window.all_data = JSON.parse(localStorage.getItem("all_data")) || {};
...
for (i = 0; i < users.length; i++) {
    if (users[i].role.toLowerCase() === "customer") {
        ...
        <td data-label="Name"><i class="fa-solid fa-circle-user"></i> ${users[i].firstName + " " + users[i].lastName}</td>
        <td data-label="Email">${users[i].email}</td>
        <td data-label="Address">${users[i].address}</td>
        </tr>`;
    }
}
