  ...
  function filterProducts() {
    ...
    filtered.forEach((product) => {
      const avgRating =
        ...
        product.ratings.length.toPrecision(2);
      ...
      if (halfStar)
        starsHtml += `<i class="fa-solid fa-star-half-stroke star"></i>`;
            ...
            <div class="col-12 col-sm-5 col-md-4 col-lg-3 col-xl-2 mb-4">
                    <div class="card h-100 shadow product-card products-all d-flex flex-column justify-content-between">
                        ...
                        <div class="card-body d-flex flex-column">
                            ...
                            <b class="card-text" style="font-size: 15px;">$${
                              product.price
                            }</b>
                        ...
                        <div class="action-buttons text-center pb-2 ">
                            <button class="btn btn-primary badge rounded-pill" onclick="editProduct(${
                              product.id
                            })">Update</button><br>
                            <button class="btn btn-danger mt-2 badge rounded-pill" onclick="deleteProduct(${
                              product.id
                            })">Delete</button>
                        </div>
                    </div>
...
window.addProduct = function () {
  ...
  if (loinUser.status == "Active") {
    ...
    Swal.fire({
      icon: "warning",
      title: "warning",
      text: "You are Banned.You can not add a product.",
    });
  }
};
...
function editProduct(productId) {
  ...
  if (product) {
    ...
    document
      .getElementById("editImageUpload")
      .addEventListener("change", function (e) {
        ...
        files.forEach((file) => {
          ...
          reader.onload = function () {
            ...
            wrapper.appendChild(deleteBtn);
            document
              .getElementById("previewContainerUpdate")
              .appendChild(wrapper);
          };
          reader.readAsDataURL(file);
        });
...
function saveProductEdits(productId) {
  ...
  if (productIndex !== -1) {
    ...
    const images = Array.from(
      document.querySelectorAll("#previewContainerUpdate img")
    ).map((img) => img.src);
    ...
    const selectedSizes = Array.from(
      document.querySelectorAll('input[name="editSizes"]:checked')
    ).map((cb) => cb.value);
